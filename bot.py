...
class ConfirmView(discord.ui.View):
    ...
    async def confirm_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if interaction.user.id != OWNER_ID:
            return await interaction.followup.send("❌ Vous n'êtes pas autorisé.", ephemeral=True)
        ...
        try:
            ...
            return await interaction.followup.send(f"❌ Utilisateur introuvable. ({e})", ephemeral=True)
...
@bot.tree.command(name="whitelist", description="[OWNER] Ajouter manuellement un utilisateur")
@app_commands.describe(
    membre="Le membre à whitelister",
...
async def whitelist(interaction: discord.Interaction, membre: discord.Member, heures: int):
    ...
    if heures < 1:
        return await interaction.followup.send("❌ Minimum 1 heure.", ephemeral=True)
    ...
    try:
        ...
        dm_embed.add_field(name="⏱ Durée", value=f"{heures} heure(s)", inline=True)
        dm_embed.add_field(name="⌛ Expire à", value=f"<t:{int(expires_at.timestamp())}:F>", inline=True)
    ...
    if channel:
        if remaining == 0:
            ...
            announce_embed = discord.Embed(
                title="🟢 Slot activé !",
                description=f"Un slot vient d'être attribué.\n**Il reste `{remaining}` slot(s) disponible(s).**",
                color=0x00FF88
            )
        await channel.send(embed=announce_embed)

    await interaction.followup.send(
        f"✅ {membre.mention} whitelisté pour `{heures}h` ! Expire <t:{int(expires_at.timestamp())}:R>",
        ephemeral=True
    )
