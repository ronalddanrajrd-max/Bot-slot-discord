...
async def whitelist_cmd(interaction: discord.Interaction, membre: discord.Member, heures: int):
    ...
    await interaction.response.send_message(f"✅ Slot de `{heures}h` attribué à {membre.mention} !", ephemeral=True)
...
async def unwhitelist_cmd(interaction: discord.Interaction, membre: discord.Member):
    ...
    if len(new_slots) == len(data["slots"]):
        return await interaction.response.send_message(f"❌ {membre.mention} n'a pas de slot actif.", ephemeral=True)
    ...
    if channel:
        remaining = slots_remaining()
        embed = discord.Embed(title="🟢 Un slot vient de se libérer !", description=f"**`{remaining}` slot(s)** disponible(s) !", color=0x00FF88)
        await channel.send(embed=embed)
    await interaction.response.send_message(f"✅ Slot retiré pour {membre.mention}.", ephemeral=True)
